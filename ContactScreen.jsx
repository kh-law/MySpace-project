...
function ContactScreen({ onSent }) {
  const rows = [
    ...
    ["phone", "Телефон", "+38 099 288 98 72"],
    ["instagram", "Instagram", "myspacerv"],
    ["clock", "Години", "Щодня · 12:00–02:00"]
  ];
  return (
    <Section tight>
      <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: "var(--space-16)", alignItems: "start" }}>
        <div style={{ display: "flex", flexDirection: "column", gap: "var(--space-10)" }}>
          <SectionHeading eyebrow="Контакти" title="Ми на сьомому поверсі" description="Вхід через головний лобі «Skyliner», ліфтом до секції С." />
          <div style={{ display: "flex", flexDirection: "column", gap: "var(--space-5)" }}>
            {rows.map(([ic, k, v]) => (
              <div key={k} style={{ display: "flex", gap: "var(--space-5)", alignItems: "flex-start", paddingBottom: "var(--space-5)", borderBottom: "1px solid var(--border-hairline)" }}>
                ...
                <span style={{ display: "flex", flexDirection: "column", gap: "var(--space-1)" }}>
                  ...
                  <span style={{ font: "var(--type-body)", color: "var(--text-display)" }}>{v}</span>
                </span>
              </div>
            ))}
          </div>
